...
export default function Products() {
  ...
  // Memoize filtered products with enhanced search
  ...
  const handleAddToCart = async (product: Product) => {
    ...
    try {
      ...
      setCartQuantity(1); // Reset quantity after adding to cart
    } catch (error: any) {
    ...
    try {
      ...
      reader.onload = async (event) => {
        try {
          ...
          for (const row of jsonData as any[]) {
            try {
              if (!row.nome || !row.codigo || !row.marca) {
                errors.push(`Linha com campos obrigatórios faltando`);
                continue;
              }
              ...
              if (existingProduct) {
                errors.push(`Produto "${row.nome}" já existe`);
                continue;
              }
              ...
              if (insertError) {
                errors.push(`Erro ao criar produto "${row.nome}": ${insertError.message}`);
              } else {
                created++;
              }
            } catch (error: any) {
          ...
          if (created > 0) {
            toast.success(`${created} ${created === 1 ? 'produto criado' : 'produtos criados'} com sucesso!`);
            fetchProducts();
          }
          ...
          >
            <div className="flex items-start space-x-4">
              ...
              <div className="flex items-center space-x-2 ml-4">
                {/* Add to Cart Button for customers and sellers */}
                {(user?.role === 'customer' || user?.role === 'seller') && (
                  ...
                  >
                    <ShoppingCart className={`h-5 w-5 ${addingToCart === product.id ? 'animate-pulse' : ''}`} />
                  </button>
                )}
