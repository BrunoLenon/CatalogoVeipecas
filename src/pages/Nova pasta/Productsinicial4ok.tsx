...
export default function Products() {
      ...
      for (let i = 0; i < itemsToImport.length; i++) {
        ...
        try {
          if (!row.nome || !row.codigo || !row.marca) {
            errors.push(`Linha ${i + 2}: Campos obrigatórios (nome, código, marca) faltando`);
            continue;
          }
          ...
          if (existingProduct) {
            ...
            if (insertError) {
              errors.push(`Erro ao criar "${row.nome}": ${insertError.message}`);
            } else {
              created++;
            }
          }
  ...
  const FuturisticProgressIndicator = ({ progress }: { progress: number }) => {
    const circumference = 2 * Math.PI * 90; // Raio de 90px
    ...
    return (
      <div className="relative flex items-center justify-center">
        ...
        {/* Texto central com efeito neon */}
        <div className="absolute flex flex-col items-center">
          <span className="text-4xl font-bold text-white drop-shadow-[0_0_10px_rgba(0,204,255,0.8)]">
            {progress}%
          </span>
          <span className="text-sm text-gray-300 mt-2 animate-pulse">Importando...</span>
        </div>
        ...
        {isImportModalOpen && (
          ...
          >
            ...
            >
              ...
              />
              {isImporting && !isPreviewModalOpen && (
                <div className="flex justify-center mb-4">
                  <FuturisticProgressIndicator progress={importProgress} />
                </div>
              )}
